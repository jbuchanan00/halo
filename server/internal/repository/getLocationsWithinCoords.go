...
func GetLocationsWithinCoords(db *pgxpool.Pool, maxLat float32, minLat float32, maxLong float32, minLong float32) []*app.Location {
	...
	sqlQuery := "SELECT id, city_ascii as name, state_id as state, lat as latitude, lng as longitude, ranking FROM location WHERE (lat BETWEEN $1 and $2) and (lng BETWEEN $3 and $4) ORDER BY ranking"

	rows, err := db.Query(context.Background(), sqlQuery, float64(minLat), float64(maxLat), float64(minLong), float64(maxLong))

	if err != nil {
		log.Printf("Error retrieving locations within coordinates %s", err)
	}
